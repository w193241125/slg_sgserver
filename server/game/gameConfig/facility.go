...
type facility struct {
	...
	Type       int8         `json:"type"`       //类型, 比如疾风营
	Additions  []int8       `json:"additions"`  //建造后得到的属性, 比如获得速度 TypeSpeed
	Conditions []conditions `json:"conditions"` //升级条件
	Levels     []fLevel     `json:"levels"`     // 升级所需资源
}
...
type facilityConf struct {
	Title     string             `json:"title"`
	List      []conf             `json:"list"`
	facilitys map[int8]*facility // map 的key 就是 type 类型, 比如type 是 疾风营
}
...
func (f *facilityConf) Load() {
	...
	for _, file := range files {
		if file.IsDir() || file.Name() == "facility.json" {
			continue
		}
